...
const Reports: React.FC = () => {
  ...
  const filteredRecords = useMemo(() => {
    ...
    return sorted.filter(record => {
      ...
      const collaboratorMatch = !collaboratorSearch || record.collaborator_name.toLowerCase().includes(searchLower);
  ...
  }, [records, collaboratorSearch, startDate, endDate]);
  ...
  const handleDeleteRecord = async (recordId: string) => {
    if (window.confirm('¿Estás seguro de que deseas eliminar este registro? Esta acción no se puede deshacer.')) {
      try {
        await dbService.deleteAttendanceRecord(recordId);
        loadData();
      } catch (error) {
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div>
                ...
                <input 
                  ...
                  onChange={e => { setCollaboratorSearch(e.target.value); setCurrentPage(1); }} 
              ...
              <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                ...
                <tbody className="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                  {isLoading ? (
                      ...
                      <tr key={record.id}>
                        ...
                        <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                          <button
                            ...
                            className="text-red-600 hover:text-red-900 dark:text-red-400 dark:hover:text-red-300 p-2 rounded-lg hover:bg-red-50 dark:hover:bg-red-900/30 transition-colors"
                    ...
                    <tr><td colSpan={6} className="text-center py-8 text-gray-500 dark:text-gray-400">No hay registros que coincidan con los filtros seleccionados.</td></tr>
                  )}
                </tbody>
              </table>
            </div>
